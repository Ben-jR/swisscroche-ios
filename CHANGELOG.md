...
- Correct sheet binding assignments in HomeNavigationView
...
- Add Forgejo issue templates for bug reports, feature requests, and pull requests
- Add `DynamicTypeSize+LargeText` extension for detecting large text and accessibility sizes
- Add `Cache` helper class for NSCache-based object storage with generic Key/Value support
- Add `BlockerStatusHapticModifier` for haptic feedback based on blocker extension status
- Add `CopyableTextViewModifier` for copy-to-clipboard context menu functionality
- Add comprehensive contributing guidelines with building instructions and code formatting info

### Changed

- Migrate domain from saracroche.org to app.saracroche.org across all API endpoints, associated domains, and extension configurations
- Lower iOS deployment target from 18.2 to 15.6
- Refactor pattern reset logic to unify iOS version change and expiration checks into single method
- Adjust pattern reprocess interval from 7 days to 1 day
- Increase pattern reset percentage from 5% to 10%
- Extend donation dismiss interval from 20 to 30 days
- Increase intent timeout delay from 20s to 25s
- Enhance accessibility across Settings, Lists, Home, and Report views for VoiceOver and Dynamic Type compatibility
- Improve number formatting and vocalization with spelled-out French numbers for screen readers
- Add singular/plural number handling in list displays

### Fixed

- Fix accessibility issues in SettingsNavigationView with proper link traits for external URLs
- Fix PatternRow accessibility for VoiceOver and Dynamic Type compatibility
- Fix download journey accessibility in UpdateInProgressSheet with improved text sizing and screen reader support
- Fix ReportChoiceButton vocalization and description display
- Fix HomeDisabledCard haptic feedback and vocalization for blocker status states
